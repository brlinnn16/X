...
async def _(event):
    if ispay2[0] == "yes":
        ...
        choice = str(msg[1])
        trys = 0
        await event.edit(f"ᤑ بدأ الصيد نوع  `{choice}` على `{ch}` , بعدد `{msg[0]}` من المحاولات !")

        @fifthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة الصيد"))
        async def _(event):
            if ispay2[0] == "yes":
                if "on" in isclaim:
                    await event.edit(f"ᤑ الصيد وصل لـ({trys}) من المحاولات")
                elif "off" in isclaim:
                    await event.edit("ᤑ لايوجد صيد شغال !")
                else:
                    await event.edit("ᤑ خطأ")
            else:
                pass
...
''')
    ...
    {str(eee)}''')
        ...
        isclaim.append("off")
        trys = ""
        await event.client.send_message(event.chat_id, "انتهى الفحص)") 
...
async def _(event):
    if ispay2[0] == "yes":
        ...
        if msg[0] == "تلقائي":  # تثبيت تلقائي عدد يوزر قناة
            ...
            username = str(msg[2])
            ch = str(msg[1])
            await event.edit(f"ᤑ سوف اقوم بتثبيت `{username}` على `{ch}` , بعدد `{msg[0]}` من المحاولات !")

            @fifthon.on(events.NewMessage(outgoing=True, pattern=r"\.حالة التثبيت التلقائي"))
            async def _(event):
                if "on" in isauto:
                    msg = await event.edit(f"ᤑ التثبيت وصل لـ({trys}) من المحاولات")
                elif "off" in isauto:
                    await event.edit(" ᤑ لايوجد تثبيت شغال !")
                else:
                    await event.edit(" ᤑ خطأ")
            for i in range(int(msg[0])):
                if ispay2[0] == 'no':
...
>
    ...
    {str(eee)}''')
            ...
            isclaim.clear()
            isclaim.append("off")
            await fifthon.send_message(event.chat_id, "انتهى الفحص)")
        if msg[0] == "يدوي":  # تثبيت يدوي يوزر قناة
            await event.edit(f"ᤑ حسناً سأحاول تثبيت `{username}` على `{ch}` !")
            msg = ("".join(event.text.split(maxsplit=1)[1:])).split(" ", 1)
            username = str(msg[0])
            ch = str(msg[1])
            try:
